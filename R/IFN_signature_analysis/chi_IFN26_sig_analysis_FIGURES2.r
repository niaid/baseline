...
info = info %>% mutate(Response = factor(Response, levels=c("low","high")))
...
for (tp in c(0,-7,70)) {
  ...
  w.pv = wilcox.test(X[Y=="low"], X[Y=="high"], exact=F)$p.value
  
  df.text = rbind(df.text, 
                  data.frame(day.label = day.pt, label.auc=sprintf("AUC = %.2f", r$auc), 
...
# output scores
fn.out = file.path(PROJECT_DIR, "generated_data", "IFN26", "CHI_IFN26_ge_sig_score.txt")
...
fn.fig = file.path(PROJECT_DIR, "figure_generation", "IFN26", "CHI_IFN.26gene.sig_vs_response_3time")
