...
for (tp in c(0,-7,70)) {
  ...
  tmp = data.frame(day = paste0("day ", tp), day.label=day.pt, subject=tinfo$subject[ihl], 
                   score=X, Gender=factor(Y), Response=factor(tinfo$Response[ihl]))
  df = rbind(df, tmp)
}
...
fn.fig = file.path(PROJECT_DIR, "figure_generation", glue::glue("CHI_TGSig_vs_gender_BL_HL.only"))
