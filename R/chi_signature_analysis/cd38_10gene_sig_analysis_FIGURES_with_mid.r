...
for (tp in c(0,-7,70)) {
  ...
  w.pv = jonckheere.test(X, as.numeric(Y), alternative="inc")$p.value
...
# Generate boxplots
ggplot(df, aes(x=Response, y=score, group=Response)) +
  ...
  scale_fill_manual(values=c("grey70", "grey90","grey50", "black", "white","grey35"), name="Response",
...
fn.fig = file.path(PROJECT_DIR, "figure_generation", "CD38.10gene.sig_vs_response_3time_with_mid_Johckheere")
ggsave(paste0(fn.fig, ".png"), w=7,h=3.5)
ggsave(paste0(fn.fig, ".pdf"), w=7,h=3.5)
